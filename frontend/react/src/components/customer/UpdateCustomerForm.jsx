...
const MyDropzone = ({customerId, fetchCustomers}) => {
    const onDrop = useCallback(acceptedFiles => {
        ...
        uploadCustomerProfilePicture(customerId, formData)
            ...
            .catch((err) => {
                errorNotification("Failed", "Profile picture failed uploaded")
            })
    }, [])
    const {getRootProps, getInputProps, isDragActive} = useDropzone({onDrop})

    return (
        <Box {...getRootProps()} w={'100%'} textAlign={'center'} border={'dashed'} borderColor={'gray.200'} p={6}
             rounded={'md'} borderRadius={'3xl'}>
            <input {...getInputProps()} />
            {
                isDragActive ?
...
const UpdateCustomerForm = ({fetchCustomers, initialValues, customerId}) => {
    return (
        <>
            <VStack spacing={'5'} marginBottom={'5'}>
            ...
            <Formik
                ...
                validationSchema={Yup.object({
                    ...
                    email: Yup.string()
                        ...
                        .required('Required'),
                    age: Yup.number()
                        .min(16, 'Must be at least 16yo')
                        .max(100, 'Must be less than 100yo')
                        .required('Required'),
                    gender: Yup.string()
                        .oneOf(
                            ['MALE', 'FEMALE'], 'Invalid gender'
                        )
                        .required('Required')
                })}
                validateOnMount
